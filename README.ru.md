...
- **Файловый браузер.** Открывается в той папке, где панель сейчас находится, а не в `$HOME` — путь берётся из сигнала OSC 7 от шелла либо из `pane_current_path` на tmux-панелях. Сортировка по имени, размеру или дате (по умолчанию сначала новые; выбор запоминается). Удаление файла, симлинка или пустой папки по ✕ в строке — с подтверждением в два шага и никогда не рекурсивно.
