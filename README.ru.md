...
- **Постоянные сессии (опционально).** `tmux 3.4+` рекомендуется на
  удалённом хосте. В старых версиях tmux в OSC 52 payload попадает
  лишняя клетка под курсором; websh определяет версию на attach
  (OSC 1338 от обёртки) и автотриммит trailing-символ при копировании.
  См. [`docs/persistent-sessions.md`](docs/persistent-sessions.md).
